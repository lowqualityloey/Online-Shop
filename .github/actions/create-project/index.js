...
async function run() {
  ...
  // Put in a try catch while testing, because labels might already exist
  try {
    await Promise.all(
      projectData.labels.map(async ({ name, color, description }) => {
        const issue = await octokit.issues.createLabel({
          owner,
          repo,
          name,
          color,
